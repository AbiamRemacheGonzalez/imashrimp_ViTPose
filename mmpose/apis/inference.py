...
def _inference_single_pose_model_for_imashrimp(model,
                                               img_or_path,
                                               depth_or_path,
                                               bboxes,
                                               dataset='TopDownCocoDataset',
    ...
    with torch.no_grad():
        ...
        print(f'\t\tInference model: {duration_ms:.2f} ms')
...
def inference_top_down_pose_model_for_IMASHRIMP(model,
                                                img_or_path,
                                                depth_or_path,
                                                person_results=None,
                                                bbox_thr=None,
    ...
    person_results = [{'bbox': np.array([0, 0, 1920, 1080])}]
    bboxes = np.array([box['bbox'] for box in person_results])
    bboxes_xywh = bboxes
    bboxes_xyxy = _xywh2xyxy(bboxes)

    poses, _ = _inference_single_pose_model_for_imashrimp(
        model,
        img_or_path,
        depth_or_path,
        bboxes_xywh,
...
def vis_pose_result(model,
    ...
    if dataset_info.dataset_name.startswith('camaron'):
        img = _get_path(img)
